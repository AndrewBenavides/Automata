...
global col_Path := 2
global col_Description := 3
global col_CustodianOverride := 4
global col_Status := 5
global cell_Client := "$B$2"
global cell_Project := "$B$3"
global cell_TaskTable := "$B$5"

class DiscoveryJobLog {
	__New() {
...
class DiscoveryJobLogEntry {
	__New(jobLog, row) {
		this.RowNumber := row
		this.JobLog := jobLog
		this.Worksheet := this.JobLog.Worksheet
		this.JobName := this.GetValue(col_JobName)
		this.Path := this.GetValue(col_Path)
		this.Description := this.GetValue(col_Description)
